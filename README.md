...
- 팀 구성
  - 윤예찬, 윤면진, 오찬석, 김선천, 이가희 
- 프로젝트 설명
  - SK Rookies 교육 중 머신러닝을 배운 후 '**캐글 플레이그라운드 메달따기**'라는 도전과제가 주어졌다. 데이터 분석, 전처리, 머신러닝 학습을 하여 데이터 예측을 진행한다. 예측결과 정답이 많다면 높은 점수를 얻고 메달을 딸 수 있다.

## 내가 맡은 역할:
- 내 역할 및 책임
  - 와인 품질 데이터, 신용카드 사기, 뇌졸중 확률 예측 중 **뇌졸중 예측을 담당**하였다.
 - 수행한 작업 내용
   - 데이터 분석 : 그래프를 그려 이상치 탐지
   - 데이터 전처리 : 이상치 제거, 인코딩, 교차검증
   - 머신러닝 학습 : 하이퍼파라미터, GridSearchCV, Bayesian Optimization 사용
   - 정답 예측
 - 기여한 부분
   - 점수 public = 0.87380, private = 0.89962 
 - 달성한 결과/성과
   - **770팀 중 6등 금메달**
